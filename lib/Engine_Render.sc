...
Engine_Render : CroneEngine {
	// <Render>
	var renDiskBus=Array.newClear(2);
	var renDiskSyn=Array.newClear(2);
	var renDiskBuf=Array.newClear(2);
	var renInput;
	var renBass;
	// </Render>
	// <mx>
	...
	*new { arg context, doneCallback;
	...
	alloc {
		...
		SynthDef("synthy",{
			arg out=0,diskout,hz=220,amp=1.0,gate=1,sub=0,portamento=1,bend=0,
		...
		SynthDef("icarus",{
			arg out=0,diskout,hz=220,amp=1.0,gate=1,sub=1.0,portamento=1,bend=0,
		...
		SynthDef("mdapiano",{
			arg out=0,diskout,hz=220,amp=1.0,gate=1,sub=0,portamento=1,
			attack=0.01,decay=0.2,sustain=0.9,release=5,
			mod1=0,mod2=0,mod3=0,mod4=0,pan=0,duration=600;
			var snd,env,tuning;
			mod1=Lag.kr(mod1);mod2=Lag.kr(mod2);mod3=Lag.kr(mod3);mod4=Lag.kr(mod4);
		...
		SynthDef("piano",{
			arg out=0,diskout,hz=220,amp=1.0,pan=0,gate=1,
		...
		renDiskBus = Array.fill(2,{Bus.audio(context.server,2)});
		...
		this.addCommand("input","i",{ arg msg;
			var i=msg[1];
			renInput[i]=Synth.head(nil,"renderInput",[\ch,i,\out,0,\diskout,renDiskBus[0]]);
		});
		...
		this.addCommand("bass","ffffffi",{arg msg;
			if (renBass.isNil,{
				renBass=Synth.head(nil,"bass",[\out,0,\diskout,renDiskBus[0],\amp,0]);
		...
		this.addCommand("record_start","si",{ arg msg;
			var i=msg[1];
			if (renDiskSyn[i]==nil,{
