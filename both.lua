-- render both
-- internal bass and pad
-- lead and arp are left/right mono
-- recorded into three separate tracks
...
function init()
  ...
  params:add_control("crow_decay","decay",controlspec.new(0.005,10,'lin',0.005,0.2,'s',0.005/10))
  params:add_control("crow_sustain","sustain",controlspec.new(0.005,10,'lin',0.005,10,'volts',0.005/10))
  params:add_control("crow_release","release",controlspec.new(0.005,10,'lin',0.005,0.7,'s',0.005/10))
...
function update_crow()
  crow.output[2].action="{ adsr("..params:get("crow_attack")..","..params:get("crow_decay")..","..params:get("crow_sustain")..","..params:get("crow_release")..",'sine')}"
end
  ...
  clock.run(function()
    while true do
      ...
      if debounce_crow>0 then
        print("debouncing crow "..debounce_crow)
